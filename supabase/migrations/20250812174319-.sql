...
AS $$
BEGIN
  -- This would update a downloads_count column if we had one
  -- For now, we'll just track in the downloads table
  RETURN NEW;
END;
$$;

-- Create trigger for download tracking
CREATE TRIGGER on_project_download
  AFTER INSERT ON public.project_downloads
  FOR EACH ROW EXECUTE FUNCTION public.increment_project_downloads();
